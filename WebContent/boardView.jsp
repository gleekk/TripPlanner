...
<body>
	...
	<div class="container" id="body">
		...
		<table class="table table-bordered table-hover" style="text-align:center; border: 1px solid #dddddd">
			<thead>
				<tr>
					<th colspan="8"><h4>여행지 추천 게시판</h4></th>
				</tr>
				<tr>
					<th style="background-color:#fafafa; color:#000000; width:70px;"><h5>번호</h5></th>
					<th style="background-color:#fafafa; color:#000000; width:100px;"><h5>도시</h5></th>
					<th style="background-color:#fafafa; color:#000000; width:220px;"><h5>관광지</h5></th>
					<th style="background-color:#fafafa; color:#000000; width:350px;"><h5>제목</h5></th>
		...
		<c:forEach var="board" items="${requestScope.boardList}" varStatus="status">
			<tr>
				...
				<a href="boardShow.board?boardID=${board.boardID}">
				
			<c:forEach begin="1" end="${board.boardLevel}" varStatus="status">
		 		<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span>
			</c:forEach>
			
			<c:choose>
				<c:when test="${board.boardAvailable==0}" >
					(삭제된 게시물입니다.)
				</c:when>
				<c:otherwise>
					${board.boardTitle}
				...
				<tr>
					<td colspan="8"><a href="boardWrite.board" class="btn btn-success pull-right" type="submit">글쓰기</a>
					<ul class="pagination" style="margin: 0 auto;">
	
			 <c:choose>
				<c:when test="${requestScope.startPage != 1}" >
					<li>
						<a href="boardView.board?pageNumber=${requestScope.startPage-1}" >
			...
			<c:forEach var="num" begin="${requestScope.startPage}" end="${requestScope.pageNumber-1}" step="1" varStatus="status">
				<li>
					<a href="boardView.board?pageNumber=${num}&searchType=${requestScope.searchType}">${num}</a>
				</li>
			</c:forEach>
		
			<li class="active"><a class="btn btn-success" href="boardView.board?pageNumber=${requestScope.pageNumber}&searchType=${requestScope.searchType}">${requestScope.pageNumber}</a></li>
			...
			<c:choose>
				<c:when test="${(requestScope.pageNumber+requestScope.targetPage) > (requestScope.startPage+9)}" >
					<li>
						<a href="boardView.board?pageNumber=${requestScope.startPage+10}">
		...
		<form action="boardSearch.board" method="get" class="form-inline mt-3">
