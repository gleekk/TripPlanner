...
<body>
	...
	<div class="container" id="body">
		<table class="table table-bordered table-hover" style="text-align:center; border: 1px solid #dddddd">
			<thead>
				<tr>
					<th colspan="6"><h4>자유게시판 게시물 보기</h4></th>
				</tr>
				<tr>
					<td style="background-color:#fafafa; color:#000000; width:80px;"><h5>분류</h5></td>
					<td style="width:150px;"><h5>${requestScope.board.mycategory}</h5> </td>
					...
					<c:choose>
						<c:when test="${requestScope.board.myplan ==0}" >
							일정 없음
						</c:when>
						<c:otherwise>
							${requestScope.plan.city} ${requestScope.plan.nofday-1}박${requestScope.plan.nofday}일 일정 (${requestScope.plan.departDay}~${requestScope.plan.returnDay})
						</c:otherwise>
					</c:choose>
				...
				<tr>
					...
					<td colspan="3" ><h5>${requestScope.board.myboardTitle}</h5> </td>
					<td style="background-color:#fafafa; color:#000000; width:80px;">
					<a onclick="return confirm('추천하시겠습니까?')" href="./likeAction.myboard?myboardID=${requestScope.board.myboardID}&pagename=myboard"><h5>좋아요</h5></a></td>
					<td><h5>${requestScope.board.mylikeCount} </h5> </td>
				</tr>
				<tr>
					<td style="background-color:#fafafa; color:#000000; width:80px;"><h5>작성날짜</h5></td>
					<td colspan="3" ><h5>${requestScope.board.myboardDate}</h5> </td>
					<td style="background-color:#fafafa; color:#000000; width:80px;"><h5>조회수</h5></td>
					<td><h5>${requestScope.board.myboardHit} </h5> </td>
				</tr>
				<tr>
					<td style="vertical-align: middle; min-height:400px; background-color:#fafafa; color:#000000; width:80px;"><h5>글 내용</h5></td>
					<td colspan="5" style="text-align: left; height : 400px;"><h5>${requestScope.board.myboardContent}</h5> </td>
				</tr>				
			...
			<tbody>
				<tr>
					...
					<a href="boardView.myboard" class="btn btn-success ">목록</a>					
					<c:if test="${ sessionScope.userID == requestScope.board.userID }" >
					
					<a href="boardUpdate.myboard?myboardID=${requestScope.board.myboardID}" class="btn btn-success ">수정</a>
<a href="boardDelete.myboard?myboardID=${requestScope.board.myboardID}&pagename=myboard" class="btn btn-danger" onclick="return confirm('정말로 삭제 하시겠습니까?');" >삭제</a>					
					</c:if>
					</td>					
				</tr>
			</tbody>	
	...
	<form class="form-horizontal" method="post" action="comment.myboard" >
		<input type="hidden" name="myboardID" value="${requestScope.board.myboardID}">
	
		<div class="form-group">
			<label>댓글:</label>
			<textarea class="form-control" rows="3" id="commentContent" name="mybcomment"></textarea><br>
		...
		<div class="container">
			...
			<div class="form-group">
				...
				<label>작성자:${comments.userID }</label>	
		  	 	<p class="card-text">${comments.mybcomment}</p><br>
		  	 	<c:if test="${comments.userID == sessionScope.userID }" >
				<a onclick="return confirm('정말로 삭제 하시겠습니까?')" href="delcomment.myboard?mycommentsNo=${comments.mycommentsNo}&myboardID=${comments.myboardID}" class="btn btn-danger pull-right">삭제</a>
				</c:if>
			</div>
		</div>
