...
<script type="text/javascript">
		...
		$.ajax({
			...
			success : function(result) {
				if (result == -1) {
					...
					var cityEng = data.cityEnglishName; // 검색중인 도시 영어 이름
					$('#keyword2').val(cityEng);
				}
			}
		});
	}
	...
	function HotelSearchFunction() {
		...
		var url = 'https://store.naver.com/hotels/list?destination=place:'
				+ keyword + '&checkin=' + come_date + '&checkout=' + leave_date
				+ '&rooms=' + people;
	...
	<div class="row">
		...
		<div class="col-md-8">
			...
			<input type="text" name="search" id="keyword" 
				 class="form-control input-lg" Style="margin: 1em; width:500px; " 
				placeholder="숙소를 검색하세요." onkeyup="cityfindFunction();" value="">
			<input type="hidden" id="keyword2" value="">
			</div>
	<div class="form-inline" Style="margin: 1em; text-align: center;">
			<div class="form-group" Style="margin: 1em;">
				<label for="exampleInputEmail3">출발일</label> 
				<input class="form-control input-lg" type="date" name="come_date"id="come_date">
			</div>
			<div class="form-group" Style="margin: 1em;">
				<label  for="exampleInputPassword3">귀국일</label>
				<input class="form-control input-lg" type="date" name="leave_date" id="leave_date">
			</div>
	</div>
	<div class="form-inline" Style="margin: 1em; text-align: center;">
			...
			<button class="btn btn-success btn-lg" data-toggle="collapse"
				data-target="#demo3" onclick="HotelSearchFunction();" type="button">숙소
				검색</button>
	</div>
